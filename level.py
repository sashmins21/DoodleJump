...
class Coins(Sprite):
	...
	def __init__(self, parent:Sprite, color=config.YELLOW):

		self.parent = parent
		super().__init__(*self._get_initial_pos(),
				   Coins.WIDTH, Coins.HEIGHT, color)
		
	def _get_initial_pos(self):
		x = self.parent.rect.centerx - Coins.WIDTH//2
		y= self.parent.rect.y - Coins.HEIGHT - 20
		return x,y
...
class Platform(Sprite):
	...
	def __init__(self, x:int, y:int, width:int, height:int,
			initial_bonus=False,initial_coins=False,breakable=False):
	...
	# Public getter for __coins so it remains private
	@property
	def coins(self):return self.__coins

	def add_coins(self,coins_type:type) -> None:
		assert issubclass(coins_type,Coins), "Not a valid coin type !"
		if not self.__coins and not self.breakable:
			self.__coins = coins_type(self)
...
class Level(Singleton):
	...
	def __init__(self):
		...
		self.coins_platform_chance = config.COINS_SPAWN_CHANCE
		...
		if self.__platforms:
			...
			self.__platforms.append(Platform(
				...
				*self.platform_size, #                               SIZE
				...
				initial_coins=chance(self.coins_platform_chance),
