...
class Player(Sprite, Singleton):
	...
	def change_color_if_needed(self):
		if self.coins == 10:
            # Change the color to a new color when coins > 10
			new_color = (255, 0, 0)  # Red color (you can choose your own color)
			self.image.fill(new_color)
            # Update the player's rect after changing the color
			self.rect = self.image.get_rect(center=self.rect.center)
		if self.coins == 25:
            # Change the color to a new color when coins > 10
			new_color = (0, 255, 255)  # Red color (you can choose your own color)
			self.image.fill(new_color)
            # Update the player's rect after changing the color
			self.rect = self.image.get_rect(center=self.rect.center)
		if self.coins == 50:
            # Change the color to a new color when coins > 10
			new_color = (255, 215, 0)  # Red color (you can choose your own color)
			self.image.fill(new_color)
            # Update the player's rect after changing the color
			self.rect = self.image.get_rect(center=self.rect.center)
		...
		for platform in lvl.platforms:
			...
			if self._velocity.y > .5:
				...
				if platform.bonus and collide_rect(self,platform.bonus):
					...
					self.bounceSound = pygame.mixer.music.load('doodlesound.mp3')
					pygame.mixer.music.set_volume(1)
					channel1.play(pygame.mixer.Sound('doodlesound.mp3'))
				...
				if platform.coins and collide_rect(self,platform.coins):
					platform.remove_coins()
					self.coins += 1
